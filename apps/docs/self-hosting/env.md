...
| `CONTROL_PLANE_URL` | Control plane URL (agent enroll, edge register) | `http://localhost:8080` |
...
| `TUNNET_ALLOW_PRIVATE_CONTROL_ENDPOINTS` | Allow private or loopback control and management URLs. Set only in an explicitly trusted self-hosted deployment. | `1` |
...
Tunnet rejects private, loopback, link-local, and DNS-rebinding control or management endpoints by default. This prevents a local API request from turning the privileged agent into a network proxy. Local development and private self-hosted deployments must set `TUNNET_ALLOW_PRIVATE_CONTROL_ENDPOINTS=1` in the agent service environment. Redirects are never followed.
