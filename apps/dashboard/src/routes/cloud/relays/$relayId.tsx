...
export const Route = createFileRoute("/cloud/relays/$relayId")({
  component: CloudRelayDetailPage,
});
...
function CloudRelayDetailPage() {
  const { relayId } = Route.useParams();
  const navigate = useNavigate();
  const { data: relay, isPending, isError, error } = useCloudRelay(relayId);
  const { data: health } = useCloudRelayHealth(relayId);
  const mutations = useCloudRelayMutations();
  ...
  if (isError || !relay) {
    return (
      <div className="space-y-4">
        ...
        <Button nativeButton={false} render={<Link to="/cloud/relays" />}>
          Back to Cloud relays
        </Button>
      </div>
    );
  }
  ...
  return (
    <>
      <Breadcrumb>
        <BreadcrumbList>
          <BreadcrumbItem>
            <BreadcrumbLink render={<Link to="/cloud/relays" />}>
              Relays
            </BreadcrumbLink>
          </BreadcrumbItem>
          <BreadcrumbSeparator>
      ...
      <PageHeader
        title={relay.name}
        description={`${relay.region} · Cloud relay`}
        ...
        <TabsContent value="settings">
          <div className="mx-auto flex max-w-2xl flex-col gap-4">
              ...
              <CardContent>
                ...
                >
                  <div className="space-y-2">
                    <Label htmlFor="cloud-relay-settings-name">Name</Label>
                    <Input
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="cloud-relay-settings-region">Region</Label>
                    <Input
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="cloud-relay-settings-url">URL</Label>
                    <Input
            ...
            <Card className="border-destructive/30">
              ...
              <CardContent className="space-y-4">
                <p className="text-muted-foreground text-sm">
                  Deleting this Cloud relay removes it from the deployment.
                  Organizations relying on this region may lose connectivity.
                </p>
                <Button
                  variant="destructive"
      ...
      <ConfirmDialog
        ...
        title="Suspend relay"
        description={`Suspend ${relay.name}? Agents will stop using this Cloud relay.`}
