...
pub async fn authenticate_with_limit(
    ...
    if tunnet_common::validate_endpoint_id(&endpoint_id).is_err() {
    ...
    let vk = tunnet_common::signing::verifying_key_from_hex(&endpoint_id)
        .map_err(|_| AuthError(StatusCode::BAD_REQUEST, "invalid pubkey"))?;

    tunnet_common::signing::verify(&vk, method, path, ts, &body, &sig).map_err(|_| {
