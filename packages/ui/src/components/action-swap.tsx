      ...
      {cascade ? (
        <>
          ...
          <AnimatePresence initial={false}>
            <motion.span
              key={`cascade-${transitionKey ?? value}`}
              aria-hidden
              initial={reduce || !mounted.current ? false : "initial"}
              animate="animate"
              exit="exit"
        ...
        <AnimatePresence initial={false}>
          <motion.span
            key={`${animation}-${transitionKey ?? value}`}
...
export function ActionSwapButton({
  ...
  return (
    <motion.button
      ...
      onClick={(event) => {
        if (!disabled) setAnimationNonce((nonce) => nonce + 1);
        onClick?.(event);
